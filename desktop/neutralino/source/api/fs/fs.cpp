...
#define _WINSOCKAPI_
#include <windows.h>
// (MiraOS build: nieużywany ATL include usunięty — brak CString w tym pliku)
#include <shlwapi.h>
#include <winbase.h>
#include <io.h>
...
fs::FileReaderResult readFile(const string &filename, const fs::FileReaderOptions &fileReaderOptions) {
    fs::FileReaderResult fileReaderResult;
     ifstream reader(filesystem::path(CONVSTR(filename)), ios::binary | ios::ate);
    if(!reader.is_open()) {
...
bool writeFile(const fs::FileWriterOptions &fileWriterOptions) {
    ...
    ofstream writer(filesystem::path(CONVSTR(fileWriterOptions.filename)), mode);
    if(!writer.is_open())
        return false;
...
int openFile(const string &filename) {
    ...
    ifstream *reader = new ifstream(filesystem::path(CONVSTR(filename)), ios::binary);
    if(!reader->is_open()) {
        delete reader;
        return -1;
    }
