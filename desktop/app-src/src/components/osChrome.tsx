...
export function Horyzont(props: {
  ...
  const [sys, setSys] = useState<{ battery: number | null; charging: boolean; ssid: string | null }>(() => (window as any).MiraSysInfo || { battery: null, charging: false, ssid: null });
  useEffect(() => {
    const h = () => setSys({ ...((window as any).MiraSysInfo || { battery: null, charging: false, ssid: null }) });
    window.addEventListener("mira-sysinfo", h);
    return () => window.removeEventListener("mira-sysinfo", h);
  }, []);
  useEffect(() => {
    const f = () => {
      setCzas(teraz());
  ...
  return (
    <motion.div initial={{ y: -70, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.35, type: "spring", stiffness: 120, damping: 18 }}
      ...
      <button onClick={props.onPrzeglad} title="Wszystkie aplikacje" className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-full hover:bg-white/10 transition-all group">
        ...
        <span className="text-left leading-none hidden sm:block">
          <span className="block font-display font-bold text-[11px] tracking-wide">MiraOS</span>
          <span className="block text-[9px] text-white/45">pulpit</span>
        </span>
      </button>
      ...
      <span className="flex items-center gap-0.5 pl-1">
        ...
        <span className="flex items-center gap-1.5 p-2 rounded-full text-white/60" title={sys.ssid ? "Połączono z: " + sys.ssid : "Sieć"}>{props.wifi ? <Wifi size={14} /> : <WifiOff size={14} className="text-rose-300" />}{sys.ssid && <span className="hidden lg:inline text-[10px] font-medium max-w-[90px] truncate">{sys.ssid}</span>}</span>
        <span className="hidden sm:flex p-2 rounded-full text-white/60 items-center gap-1" title={sys.charging ? "Ładowanie" : "Bateria"}><BatteryMedium size={15} /><span className="text-[10px] font-mono2">{sys.battery !== null && sys.battery !== undefined ? sys.battery : "—"}</span>{sys.charging && <span className="text-[10px]">⚡</span>}</span>
...
export function Jadro(props: { otwarte: boolean; setOtwarte: (v: boolean) => void; onOpen: (a: AppId) => void; onSzukaj: () => void; ukryte: boolean }) {
  ...
  const [czas, setCzas] = useState(teraz());
  const [realApps, setRealApps] = useState<any[]>(() => (window as any).__MIRA_APPS || []);
  useEffect(() => { const iv = setInterval(() => setCzas(teraz()), 5000); return () => clearInterval(iv); }, []);
  useEffect(() => {
    const h = () => setRealApps([...((window as any).__MIRA_APPS || [])]);
    window.addEventListener("mira-apps-updated", h);
    h();
    return () => window.removeEventListener("mira-apps-updated", h);
  }, []);
  const R = typeof window !== "undefined" && window.innerWidth < 700 ? 132 : 196;
  const R2 = R + 116;
  const pierwszy = realApps.slice(0, 8);
  const drugi = realApps.slice(8, 20);
  return (
    <div className={`absolute inset-0 z-[20] flex items-center justify-center pointer-events-none transition-opacity duration-700 ${props.ukryte && !props.otwarte ? "opacity-0 scale-95" : "opacity-100"}`}>
      <div className="relative flex items-center justify-center pointer-events-auto">
        ...
        <AnimatePresence>
          {props.otwarte && (
            <>
              ...
              {drugi.length > 0 && (
                <motion.div initial={{ scale: 0.6, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.6, opacity: 0 }} transition={{ delay: 0.12 }} className="absolute rounded-full orbit-ring opacity-40" style={{ width: R2 * 2 + 40, height: R2 * 2 + 40, borderStyle: "dotted" }} />
              )}
            </>
          )}
        </AnimatePresence>
                {/* aplikacje Mira — pierścień wewnętrzny (najważniejsze) */}
        <AnimatePresence>
          {props.otwarte && pierwszy.map((a: any, i: number) => {
            const kat = (i / Math.max(pierwszy.length, 1)) * Math.PI * 2 - Math.PI / 2;
            const x = Math.cos(kat) * R, y = Math.sin(kat) * R;
            return (
              <motion.button key={"mira-" + a.name} initial={{ x: 0, y: 0, scale: 0, opacity: 0 }} animate={{ x, y, scale: 1, opacity: 1 }} exit={{ x: 0, y: 0, scale: 0, opacity: 0 }}
                transition={{ type: "spring", stiffness: 210, damping: 20, delay: i * 0.04 }}
                onClick={() => (window as any).MiraApps?.launch(a)}
                title={"Uruchom: " + a.name}
                className="absolute group flex flex-col items-center gap-1.5 w-[92px]">
                <span className="w-[58px] h-[58px] rounded-[19px] glass-soft flex items-center justify-center overflow-hidden transition-transform group-hover:scale-110 group-hover:-translate-y-1"
                  style={{ boxShadow: "0 12px 40px -10px rgba(125,249,198,.5)" }}>
                  {a.iconDataUrl
                    ? <img src={a.iconDataUrl} alt="" draggable={false} className="w-[40px] h-[40px] object-contain" />
                    : <span className="font-display font-bold text-[18px] text-white/85">{(a.name || "?").trim().charAt(0).toUpperCase()}</span>}
                </span>
                <span className="text-[10.5px] font-semibold bg-black/45 backdrop-blur px-2 py-0.5 rounded-full border border-white/10 max-w-[90px] overflow-hidden text-ellipsis whitespace-nowrap">{a.name}</span>
              </motion.button>
            );
          })}
        </AnimatePresence>
        {/* aplikacje Mira — pierścień zewnętrzny (kolejne) */}
        <AnimatePresence>
          {props.otwarte && drugi.map((a: any, i: number) => {
            const kat = (i / Math.max(drugi.length, 1)) * Math.PI * 2 - Math.PI / 2 + Math.PI / Math.max(drugi.length, 1);
            const x = Math.cos(kat) * R2, y = Math.sin(kat) * R2;
            return (
              <motion.button key={"mira2-" + a.name} initial={{ x: 0, y: 0, scale: 0, opacity: 0 }} animate={{ x, y, scale: 1, opacity: 1 }} exit={{ x: 0, y: 0, scale: 0, opacity: 0 }}
                transition={{ type: "spring", stiffness: 190, damping: 20, delay: 0.2 + i * 0.025 }}
                onClick={() => (window as any).MiraApps?.launch(a)}
                title={"Uruchom: " + a.name}
                className="absolute group flex flex-col items-center gap-1 w-[78px]">
                <span className="w-[44px] h-[44px] rounded-[15px] glass-soft flex items-center justify-center overflow-hidden transition-transform group-hover:scale-110">
                  {a.iconDataUrl
                    ? <img src={a.iconDataUrl} alt="" draggable={false} className="w-[30px] h-[30px] object-contain" />
                    : <span className="font-display font-bold text-[14px] text-white/85">{(a.name || "?").trim().charAt(0).toUpperCase()}</span>}
                </span>
                <span className="text-[9.5px] font-semibold bg-black/45 backdrop-blur px-1.5 py-0.5 rounded-full border border-white/10 max-w-[76px] overflow-hidden text-ellipsis whitespace-nowrap opacity-80">{a.name}</span>
              </motion.button>
            );
          })}
        </AnimatePresence>
        {props.otwarte && realApps.length === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute rounded-full glass-soft px-5 py-3 text-[12px] text-white/60" style={{ left: "50%", top: R + 150, transform: "translateX(-50%)" }}>
            Mira rozgląda się za Twoimi aplikacjami… chwilka ✦
          </motion.div>
        )}
        {/* centralne jądro */}
        <div className="relative flex flex-col items-center">
          ...
          {props.otwarte && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-10 flex flex-col items-center gap-2.5">
              ...
              {realApps.length > 0 && (
                <button onClick={() => (window as any).MiraOpenLauncher?.()}
                  className="flex items-center gap-2 rounded-full glass-soft px-4 py-2 text-[11.5px] text-white/55 hover:text-white transition-colors">
                  <LayoutGrid size={13} /> Wszystkie aplikacje ({realApps.length})
                </button>
              )}
            </motion.div>
          )}
...
export function Rzeka(props: {
  ...
  const pasek = realApps.slice(0, 8);
  return (
    <motion.div initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.5, type: "spring", stiffness: 110, damping: 18 }}
      className="absolute bottom-4 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-1 rounded-[26px] glass pl-2 pr-2 py-2 max-w-[96vw]">
      <button onClick={props.onJadro} title="Jądro — Twoje aplikacje" className={`w-11 h-11 rounded-full relative shrink-0 transition-transform hover:scale-105 ${props.jadroOtwarte ? "scale-105" : ""}`}>
        <span className="absolute inset-0 rounded-full orb-halo" />
      ...
      <button onClick={props.onSzukaj} title="Szukaj ( / )" className="w-11 h-11 rounded-2xl bg-white/[0.06] hover:bg-white/[0.13] border border-white/10 flex items-center justify-center text-white/70 shrink-0"><Search size={17} /></button>
      <button onClick={props.onPrzeglad} title="Wszystkie aplikacje" className="w-11 h-11 rounded-2xl bg-white/[0.06] hover:bg-white/[0.13] border border-white/10 flex items-center justify-center text-white/70 shrink-0"><LayoutGrid size={17} /></button>
      {pasek.length > 0 && <span className="w-px h-8 bg-white/10 mx-1 shrink-0" />}
      <AnimatePresence mode="popLayout">
        {pasek.map((a: any) => (
          <motion.button key={a.name} layout initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }}
            onClick={() => (window as any).MiraApps?.launch(a)} title={"Uruchom: " + a.name}
            className="relative w-11 h-11 rounded-2xl flex items-center justify-center bg-white/[0.05] border border-white/10 hover:bg-white/12 hover:-translate-y-1 transition-all shrink-0">
            {a.iconDataUrl
              ? <img src={a.iconDataUrl} alt="" draggable={false} className="w-7 h-7 object-contain" />
              : <span className="font-display font-bold text-[13px] text-white/85">{(a.name || "?").trim().charAt(0).toUpperCase()}</span>}
          </motion.button>
        ))}
      </AnimatePresence>
      {pasek.length === 0 && (
        <span className="px-3 text-[11px] text-white/35 italic whitespace-nowrap hidden sm:block">Rzeka szuka Twoich aplikacji… chwilka</span>
      )}
    </motion.div>
  );
}
...
export function Szukaj(props: { otwarte: boolean; onZamknij: () => void; onOpen: (a: AppId) => void; onAkcja: (a: string) => void }) {
  ...
  const apps = realApps.filter((a: any) => a.name.toLowerCase().includes(ql)).slice(0, 12);
  const akcje = [
    { id: "aplikacje", t: "Wszystkie aplikacje", d: "szklana siatka wszystkich programów", ikona: "▦" },
    { id: "fullscreen", t: "Pełny ekran", d: "MiraOS na całym ekranie (F11)", ikona: "⛶" },
    { id: "zorzowy", t: "Losowa zorza", d: "zmień krajobraz Miry", ikona: "✦" },
    { id: "blokada", t: "Zapaść w sen", d: "zablokuj Mirę", ikona: "◉" },
    { id: "wyjscie", t: "Zakończ MiraOS", d: "wróć do zwykłego pulpitu (Ctrl+Alt+Q)", ikona: "☾" },
    { id: "uspijpc", t: "Uśpij komputer", d: "tryb uśpienia Windows", ikona: "☾" },
    { id: "restart", t: "Uruchom ponownie", d: "restart komputera", ikona: "↻" },
    { id: "wylacz", t: "Wyłącz komputer", d: "zamknij Windows", ikona: "⏻" },
  ].filter((a) => (a.t + a.d).toLowerCase().includes(ql));
  const pierwszyWynik: any = apps[0] || akcje[0];
  return (
    <AnimatePresence>
      {props.otwarte && (
          ...
          <motion.div initial={{ y: -26, scale: 0.96, opacity: 0 }} animate={{ y: 0, scale: 1, opacity: 1 }} exit={{ y: -18, scale: 0.97, opacity: 0 }} transition={{ type: "spring", stiffness: 260, damping: 26 }}
            ...
            <div className="flex items-center gap-3 px-5 py-4 border-b border-white/10">
              ...
              <input ref={input} value={q} onChange={(e) => setQ(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter" && pierwszyWynik) { if (pierwszyWynik.target !== undefined) { (window as any).MiraApps?.launch(pierwszyWynik); props.onZamknij(); } else props.onAkcja(pierwszyWynik.id); } if (e.key === "Escape") props.onZamknij(); }}
                placeholder="Zapytaj Mirę… wpisz nazwę aplikacji" className="bg-transparent w-full text-[15px] placeholder:text-white/30" />
              <kbd className="text-[10px] font-mono2 border border-white/15 rounded-lg px-2 py-1 bg-black/30 text-white/40">esc</kbd>
            </div>
            <div className="overflow-y-auto p-3">
              {apps.length > 0 && <div className="text-[10px] uppercase tracking-[0.2em] text-white/35 font-bold px-2 pt-1 pb-2">Twoje aplikacje</div>}
              {apps.map((a: any) => (
                <button key={a.name} onClick={() => { (window as any).MiraApps?.launch(a); props.onZamknij(); }} className="w-full flex items-center gap-3 rounded-2xl p-2.5 hover:bg-white/10 text-left group">
                  <span className="w-11 h-11 rounded-2xl glass-soft flex items-center justify-center shrink-0 overflow-hidden" style={{ boxShadow: "0 6px 20px -8px rgba(125,249,198,.4)" }}>
                    {a.iconDataUrl
                      ? <img src={a.iconDataUrl} alt="" draggable={false} className="w-8 h-8 object-contain" />
                      : <span className="font-display font-bold text-[15px] text-white/85">{(a.name || "?").trim().charAt(0).toUpperCase()}</span>}
                  </span>
                  <span className="min-w-0 flex-1"><span className="block font-bold text-[13.5px] truncate">{a.name}</span><span className="block text-[11.5px] text-white/40 truncate">{a.type === "uwp" ? "aplikacja ze Sklepu Windows" : "program"}</span></span>
                  <ArrowRight size={15} className="ml-auto opacity-0 group-hover:opacity-40 transition-opacity" />
                </button>
              ))}
              ...
              {apps.length === 0 && akcje.length === 0 && (
                <div className="text-center py-10 text-white/40"><div className="text-[30px] mb-2">〜</div>Eter milczy. Nie znaleziono „{q}".</div>
              )}
            </div>
          </motion.div>
        </motion.div>
        ...
        <>
          ...
          <motion.div initial={{ x: 380 }} animate={{ x: 0 }} exit={{ x: 380 }} transition={{ type: "spring", stiffness: 240, damping: 28 }}
            ...
            <div className="mt-3 space-y-3">
              <div><div className="flex justify-between text-[12px] font-semibold"><span>Głośność Eteru</span><span className="font-mono2 text-white/40">{Math.round(props.glos * 100)}</span></div><input type="range" min={0} max={100} value={Math.round(props.glos * 100)} onChange={(e) => { props.setGlos(+e.target.value / 100); (window as any).MiraSound?.setVolume(+e.target.value / 100); (window as any).MiraSetVolume?.(+e.target.value); }} className="w-full mt-1" /></div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-3">
              <button onClick={props.onLab} className="rounded-2xl py-2.5 bg-white/10 border border-white/10 text-[12px] font-bold hover:bg-white/15 flex items-center justify-center gap-1.5"><Settings2 size={13} /> Aplikacje</button>
...
export function Boot({ postep }: { postep: number }) {
  const wskazowki = ["Twoje aplikacje czekają w Jądrze.", "Naciśnij / i wpisz nazwę programu.", "F11 — pełny ekran. Ctrl+Alt+Q — wyjście.", "Rzeka pamięta wszystko, co niosła."];
  ...
  return (
    <motion.div exit={{ opacity: 0, scale: 1.06 }} className="absolute inset-0 z-[100] bg-[#07080f] flex flex-col items-center justify-center grain overflow-hidden">
      ...
      <div className="font-display font-extrabold text-[34px] tracking-[0.3em] mt-8 ml-2">MiraOS</div>
  ...
  return (
    <motion.div exit={{ y: "-100%", opacity: 0.6 }} transition={{ type: "spring", stiffness: 90, damping: 20 }}
      ...
      <div className="absolute bottom-6 inset-x-0 flex justify-center gap-6 text-white/30 text-[11px] font-mono2">
        <span>F11 — pełny ekran</span><span>/ — szukaj</span><span>Ctrl+Alt+Q — wyjście</span>
      </div>
    </motion.div>
  );
}
...
export function MenuKontekst({ x, y, onZamknij, onAkcja }: { x: number; y: number; onZamknij: () => void; onAkcja: (a: string) => void }) {
  const opcje = [
    ...
    { id: "aplikacje", t: "Wszystkie aplikacje", ikona: "▦" },
    { id: "zorza", t: "Zmień krajobraz", ikona: "〜" },
    { id: "fullscreen", t: "Pełny ekran", ikona: "⛶" },
    { id: "wyjscie", t: "Zakończ MiraOS", ikona: "☾" },
  ];
  return (
    <>
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
        ...
        <div className="px-3 pt-2 pb-1 text-[10px] uppercase tracking-[0.2em] text-white/35 font-bold">MiraOS</div>
        {opcje.map((o) => (
